...
4. Suppose we want to convert a factor variable to a numeric variable in, how do we do so? 
For example, convert the factor variable below to numeric.  
...
# as.numeric(levels(X))[X] ## alternative
```
...
qplot(wt,mpg,data=mtcars,size = disp, color=cyl)

#ggplot(mtcars, aes(wt, mpg)) + geom_point(aes(size = disp, color=cyl))
